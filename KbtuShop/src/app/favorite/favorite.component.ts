...
export class FavoriteComponent implements OnInit {
  ...
  delete(product: Product) {
    ...
    let found = false;
    // tslint:disable-next-line: prefer-for-of
    for (let i = 0; i < this.favorite.length && !found; i++) {
        if (this.favorite[i].product === product.id) {
          favoriteId = this.favorite[i].id;
          found = true;
        }
    }
    this.favoriteService.deleteFromFavorite(favoriteId).subscribe(() => console.log('deleted'));
  }
}
