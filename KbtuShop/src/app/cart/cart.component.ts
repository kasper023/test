...
export class CartComponent implements OnInit {
  ...
  delete(product: Product) {
    ...
    let found = false;
    // tslint:disable-next-line: prefer-for-of
    for (let i = 0; i < this.cart.length && !found; i++) {
        if (this.cart[i].product === product.id) {
          cartId = this.cart[i].id;
          found = true;
        }
    }
    this.cartService.deleteFromCart(cartId).subscribe(() => console.log('deleted'));
  }
